...
class Produtos extends MX_Controller {
			...
			if (empty($data['id'])) {
				if ($this->clog_model->salvar('produtos', $data)) {
					...
					if (empty($data['imagem'])) {
						if ($this->upload->do_upload($file_name)) echo "file upload success"; 
						else {
	...
	public function consulta_estoque() {
		...
		if ($this->input->get('modelo') != "") $filter['modelo'] = $this->input->get('modelo');
		if ($this->input->get('marcas') != 0) $filter['marcas_produtos_id'] = $this->input->get('marcas');
		if ($this->input->get('almox') != 0) $filter['lotacoes_id'] = $this->input->get('almox');
	...
	public function consulta_produtos() {
		...
		if ($this->input->get('grupo_produtos') != "") { 
			$filter['grupo_produtos'] = $this->input->get('grupo_produtos'); 
		}
	...
	public function detalheProdutos($id) {
		...
		$lista_tombos = $this->cautelas_model->getTombosProduto($id, FALSE);
		if (! is_bool($lista_tombos)) {
			foreach ($lista_tombos as $tombo) {
				$tombos[$tombo] = $this->cautelas_model->getTomboInfo($tombo); 
			}
			$this->load->view('clog/produtos/detalhe_produtos', array('produtos'=>$produtos, 'tombos'=>$tombos), FALSE);
		}
		else $this->load->view('clog/produtos/detalhe_produtos', array('produtos'=>$produtos), FALSE);
	}
	...
	public function historico_produto() {
		...
		if (! is_bool($tombo)) {
			...
			if (! is_bool($historico) && ! is_bool($tombo_info)) $this->load->view('clog/produtos/historico_produto', array('historico'=>$historico->result(), 'tombo_info'=>$tombo_info), FALSE);
			else return FALSE;
		}
		else return FALSE;
	}
	...
	public function imprime_historico_produto() {
		...
		if (! is_bool($tombo)) {
			$tombo_info = $this->produtos_model->produtoByTombo($tombo);
			$historico = $this->produtos_model->historicoByTombo($tombo);
			if (! is_bool($historico) && ! is_bool($tombo_info)) $this->load->view('clog/produtos/imprime_historico_produto', array('historico'=>$historico->result(), 'tombo_info'=>$tombo_info), FALSE);
			else return FALSE;
		}
		else return FALSE;
	}
