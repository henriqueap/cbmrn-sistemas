...
class Tipo_viatura_model extends CI_Model {
	...
	public function listar() {
		...
		$_lista = $this->db->get('tipo_viaturas');
	...
	public function getById($id) {
		$this->db->where('id', $id);
		$_tipo_viatura = $this->db->get('tipo_viaturas');
		return ($_tipo_viatura->num_rows() > 0)? $_tipo_viatura->row(): FALSE;
	}

	public function testaExiste($id) {
		$this->db->where('tipo_viaturas_id', $id);
		$_existe = $this->db->get("viaturas");
