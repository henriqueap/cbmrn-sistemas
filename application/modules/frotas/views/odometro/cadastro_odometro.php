<div class="container">
	<div class="well well-cadastro">
		...
		<?php echo form_open('frotas/odometro/cadastro',array('class'=>'form-horizontal','role'=>'form')); ?>
						...
						<select class="form-control input-sm" name="selViatura" id="selViatura" required>
							<option value>Selecione</option>
								<?php foreach($listar_viaturas->result() as $viaturas) : ?>
									<option value="<?php echo $viaturas->id."-".$viaturas->tipo_viaturas_id; ?>"><?php echo $viaturas->placa." - ".$viaturas->marca." - ". $viaturas->modelo; ?></option>
								<?php endforeach; ?>
						</select>
					</div>
				</div>          
				<div class="form-group">
					...
					</div><span id="max-odometro"></span>
				</div>
				<div id="semViatura">
				<!-- 	<div class="form-group">
						<label for="inputDestino" class=" col-sm-1  control-label">Destino</label>
						<div class=" col-sm-6 ">
							<textarea class="form-control" name="inputDestino" id="inputDestino" cols="30" rows="2" placeholder="" ><?php 
				...
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4"> 
					<center>
						<button type="submit" class="btn btn-success" id="btnSalvarOdom">Enviar</button>
					</center>
				</div>
				<?php echo form_close();?>
		</div> <!--panel-->
