...
class Modelos_marcas extends MX_Controller {
	...
	function __construct() {
		...
		$_models = array(
			'frotas_model', 
			'marcas_veiculos_model', 
			'modelos_marcas_model'
		);
	...
	public function cadastro($data = "", $data2=""){
		//var_dump($data);
		$modelos = $this->modelos_marcas_model->listar(); //listar todos os modelos, com marca, do banco
		$listar_marcas = $this->marcas_veiculos_model->listar(); //retorna marcas para o select na view 
		$modelos_marcas = $this->load->view('modelos_marcas/cadastro', 
																			array(
																				'data2' => $data2, 
																				'data' => $data, 
																				'listar_marcas'=>$listar_marcas, 
																				'modelos'=>$modelos
																			), TRUE); //passando as variaveis via array para a view
	...
	public function editar($id) {
		...
		//$data2 = $this->marcas_veiculos_model->getByIdMarcas($data->marca_veiculos_id); // Passando via array marca_veiculos_id e nome herdado reutilizando o metodo do Model
		/*$data2 = array(
			'id'=>$data2['0']->id, 
			'nome'=>$data2['0']->nome
		);*/
		$this->cadastro($data, $data);
	}
