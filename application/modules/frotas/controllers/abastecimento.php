...
class Abastecimento extends MX_Controller {
	...
	public function cadastro() {
		...
		$abastecimento = $this->load->view('abastecimento/cadastro', 
			array('listar_viaturas'=>$listar_viaturas, 'setor_lotacao'=>$setor_lotacao), TRUE);
		...
		if($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			if ($militar == 0) {
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$var2 = $this->odometro_model->cadastrar($data2, TRUE);

					if($var2 != false) {
						$data = array(
							'viaturas_id' => $data2['viaturas_id'],
							'data' => $data2['data'],
							'odometros_id' => $var2,
	...
	public function salvar() {
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			// Validando form e executando os cadastros
			else {
				// Validando form
