...
class Listar_viaturas extends MX_Controller {
	...
	public function listar($id=NULL){
		$listar_lotacoes=$this->listar_viaturas_model->listarLotacoes();
		...
		$testa_inativo = $this->listar_viaturas_model->listarViaturas($id, 0);
		if ($testa_ativo !== FALSE ) {
			$listar_viaturas_ativos = $testa_ativo->result();
		}
		else $listar_viaturas_ativos = FALSE;
		if ($testa_inativo !== FALSE ) {
			$listar_viaturas_inativos = $testa_inativo->result();;
		}
		else $listar_viaturas_inativos = FALSE;
		$listar=$this->load->view('listar_viaturas/listar', array('listar_viaturas_inativos'=>$listar_viaturas_inativos,'listar_viaturas_ativos'=>$listar_viaturas_ativos,'listar_lotacoes'=>$listar_lotacoes,'listar_tipo_viaturas'=>$listar_tipo_viaturas), TRUE);
		$this->load->view('layout/index', array('layout'=>$listar,), FALSE);
	}
	...
	public function editar($id) { 
		$this->listar_viaturas['idviaturas']=$id ;       
   ...
   public function listarOdometros($id){
			  $listarOdometrosId=$this->listar_viaturas_model->listarOdometros($id);
			  $data = array(
				'id'=>$this->input->get('inputIdViaturas'), 
				'data_inicial'=>$this->input->get('dataInicial'), 
				'data_final'=>$this->input->get('dataFinal'), 				
			);			  
			  $listar=$this->load->view('listar_viaturas/listar_odometros', array('listarOdometrosId'=>$listarOdometrosId,'idViatura'=>$id), TRUE);
			  $this->load->view('layout/index', array('layout'=>$listar,), FALSE); 
   }
