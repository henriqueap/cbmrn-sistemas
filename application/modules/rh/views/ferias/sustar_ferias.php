	...
	<div class="row">
		...
		<?php echo form_open('rh/ferias/sustar_ferias', array('class'=>'form-horizontal', 'role'=>'form')); ?>
			...
			<div class="form-group">
				...
				<div class="col-sm-12 col-md-2 col-lg-2 col-xs-12">
					<!-- Listagem de todos os exercícios já cadastrados -->
					<select name="exercicio" id="exercicio" class="form-control input-sm" required>
						...
						<?php foreach ($turma_ferias as $row): ?>
							<option value="<?php echo $row->exercicio; ?>" <?php echo ($row->exercicio == date('Y') - 1)? "selected" : ""; ?>><?php echo $row->exercicio ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div><!--/.form-group-->  
