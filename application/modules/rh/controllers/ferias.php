...
class Ferias extends MX_Controller {
	...
	public function cadastro_ferias() {
		...
		if ($this->input->server('REQUEST_METHOD') == "POST") {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				# Pegando as informações da Turma de Férias
				$info_turma_ferias = $this->ferias_model->getTurmaFerias($data)->row();
				# var_dump($info_turma_ferias); die();
				...
				foreach ($novo_arr AS $value) {
					...
					$novo_data = array(
						'data_inicio'=>$info_turma_ferias->data_inicio,
						'data_fim'=>date('Y-m-d', strtotime("$info_turma_ferias->data_inicio + 29 days")),
						'tipo_afastamentos_id'=>1,
						'turma_ferias_id'=>$info_turma_ferias->id, 
						'militares_id'=>$this->militares_model->getByMatricula(trim($value), TRUE)
					);
				...
				redirect('rh/ferias/cadastro_ferias');
			} # Fim else.
		}
	...
	public function get_militares_ferias() {
		...
		$filter = array();
		# Filtros.
		if ((!$this->input->get("militar") == "") && (!$this->input->get("exercicio") == "")) {
			...
			<div class="panel-body">
				<div class="group">
					<label>Militar Inexistente ou não possui turma cadastrada neste exercício.</label>
				</div>
			</div>
			<?php
			exit();
		...
		# var_dump($info_militares); die();
		# $info_militares = $this->ferias_model->getFeriasMilitares($filter)->row();
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (count($this->ferias_model->salvar_sustar($data)) > 0) {
					$this->session->set_flashdata('mensagem', array('type' => 'alert-success', 'msg' => 'Férias sustadas com sucesso!'));
	...
	public function get_info_reaprazamento() {
		...
		if (! $this->input->get("militar") == "") {
			$filter['afastamentos.militares_id'] = $this->input->get("militar");    
		} else {
		...
		if (!$this->input->get("exercicio") == "") {
			$filter['turma_ferias.exercicio'] = $this->input->get("exercicio");
		} else exit();

		# Pegar informações sobre as férias do militar.
		$info_militares = $this->ferias_model->getAfastamentoMilitares($filter);
